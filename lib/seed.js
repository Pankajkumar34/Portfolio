...
// Sample data that matches your portfolio structure
const portfolioData = {
  hero: {
    sectionName: 'hero',
    content: {
...
export async function seedPortfolio() {
  try {
    ...
    for (const [sectionName, data] of Object.entries(portfolioData)) {
      const result = await Section.findOneAndUpdate(
        { sectionName: data.sectionName },
        data,
        { new: true, upsert: true }
      );
      console.log(`✅ Section "${sectionName}" seeded successfully`);
    }

    // Seed menu settings
    console.log('🔄 Seeding menu settings...');
    ...
    // Seed chatAI data
    console.log('🔄 Seeding chatAI data...');
